# Doc Templates 重构 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 `00-doc-templates.md` 从"场景嵌套参考书"重构为"agent 决策优先 + 扁平模板 + 校验规则"3 部分结构。

**Architecture:** 单文件重写。Part 1 类型选择表（15 行 4 列），Part 2 每类型一个扁平块（用途/必填节/可选节含触发条件/常见错误），Part 3 跨类型校验规则表（8 条）。最深 2 层（## 类型 → ### 节）。

**Tech Stack:** Markdown，无代码依赖。

**Source spec:** [doc-templates-refactor-design](../../docs/superpowers/specs/2026-05-16-doc-templates-refactor-design.md)

---

### Task 1: 重写 00-doc-templates.md

**Files:**
- Rewrite: `docs/01-old-files/00-knowledge-base-update/00-doc-templates.md`

- [ ] **Step 1: 写入新文件**

用以下完整内容替换 `docs/01-old-files/00-knowledge-base-update/00-doc-templates.md`：
...
- [ ] **Step 2: 验证 15 种类型与命名规范对齐**

打开 `docs/01-old-files/00-knowledge-base-update/00-spec-markdown-name.md`，确认受控词汇表 `类型` 列的 15 个值（architecture, standard, reference, changelog, test, solution, spec, api, benchmark, review, contract, design, protocol, requirement, guide）与模板文件的 15 个 `###` 标题一一对应，无遗漏无多余。
...
1. 无 `> 来源：` 等旧版元数据残留
2. 无"场景 N"嵌套
3. 每个类型块结构一致：用途 → 必填节 → 可选节 → 常见错误
4. 15 个 `###` 标题与类型选择表 15 行对应

- [ ] **Step 4: 提交**

```bash
git add docs/01-old-files/00-knowledge-base-update/00-doc-templates.md
git commit -m "refactor: doc-templates — flat decision-first structure for agent consumption"
```
