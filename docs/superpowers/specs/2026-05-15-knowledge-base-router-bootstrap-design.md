...
router 只决定下一个 worker 和 handoff payload，不执行 worker 职责本身。
...
| `knowledge-base-publisher` | `kb_state=Ready`、有确认草稿或明确 index-only/rename/update 目标，且写入意图已确认 | 未确认发布、分类缺失、目标冲突未确认、需要跨分类迁移/去重/归档 |
...
| `knowledge-base-publisher` | `operation`、`draft_or_target` | `category`、`filename`、`summary` |
...
- `knowledge-base-publisher.operation=add` 时必须有草稿正文、目标分类和已确认的发布意图。
...
- 看到 author 输出草稿后直接启动 publisher，而没有确认 `operation`、`draft_or_target`、目标分类和 `confirmation_status`。
