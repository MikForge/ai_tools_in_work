...
| `knowledge-base-publisher` | `kb_state=Ready`、有确认草稿或明确 index-only/rename/update 目标，且写入意图已确认 | 未确认发布、缺少草稿或目标、目标冲突未确认、需要跨分类迁移/去重/归档 |
...
`knowledge-base-router` 是唯一允许被用户、agent、workflow 或其他 skill 隐式触发的 public skill。所有 `knowledge-base-*` worker 都是同一个 package 下的 internal skill。
...
- 即使运行时不支持调用控制字段，internal worker 正文也必须拒绝缺少完整 router/auditor handoff payload 的直接外部请求。
