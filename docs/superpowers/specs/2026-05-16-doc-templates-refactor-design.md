...
**日期**：2026-05-16
**状态**：design
**目标文件**：`docs/01-old-files/00-knowledge-base-update/00-doc-templates.md`

---

## 动机

现有模板文件是"人类参考书"结构（类型→场景→必填/可选→节），agent 读取时面临：
- 4 层嵌套，定位慢
- 无类型选择指引，spec/design/solution 之间犹豫
- 节描述偏"给人看"，缺少 agent 可执行的边界条件
- 元数据噪音（来源链接、社区上下文）

目标：重构为"agent 决策工具 + 可执行模板"，同时人类可读。
...
**目标：**
- agent 在 ≤1 次查找内确定文档类型和必填节
- 每种类型一个扁平块，最深 2 层（类型→节）
- agent 写完文档可机械自检
- 结构足够扁，agent 自己能增删类型（熵管理）

**非目标：**
- 不改变命名规范（`00-spec-markdown-name.md` 不动）
- 不改变 knowledge-base 配置体系
- 不追求全面覆盖所有边缘场景
...
**可选节**：
- 技术细节 — 边界情况、与现有功能的交互（仅在有非显而易见的交互时加）
- 先前经验 — 其他项目/论文参考（仅在确实借鉴时加）
- 待解决问题 — 有意搁置的问题清单
- 未来可能 — 本提案不做的方向性扩展
...
1. 重写 `00-doc-templates.md` 为新的 3 部分结构
2. 确保 15 种类型与 `00-spec-markdown-name.md` 受控词汇表 `类型` 一一对应
3. 去掉来源链接、场景嵌套等人类导向元素
4. 自审后提交
