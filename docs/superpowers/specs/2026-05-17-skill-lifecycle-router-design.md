...
Skill 开发涉及多个阶段（设计、计划、测试、审查、反馈），但没有统一入口。用户需要自己知道每个阶段用什么工具、按什么顺序。更关键的是：各阶段孤立运行，**下游不知道上游产出了什么，反馈无法回流到源头**。
...
| Agent 框架层 | 子 skill — design / plan / test / review / feedback | Agent 如何执行具体任务 |
...
| **护栏二：架构约束** — 分层依赖 + 自动阻断 + 解释 why | design → plan → test 分层依赖链；入口条件阻断（前置产物不存在拒入并解释为什么和怎么补）；review 阶段模式一致性检查 |
| **护栏三：反馈回路** — Agent 审 Agent + 测试反查 | TDD 循环：测试审代码（第一层） + 测试有效性反查（第二层，审审查者）；review 阶段结构化审查产物；出口自检防过早宣布胜利；feedback 统一标注回流阶段 |
...
| 过早标记功能完成 | 写完代码就标记完成，从未测试 | **test 自检**要求测试通过；**测试有效性反查** — 如测试全通过但代码质量可疑，判定测试边界不足 |
...
skill-lifecycle-router/
  ...
  zh-CN.md
  constraints/                          # 审查约束规范（review 子 skill 按产物类型读取对应清单）
  ...
  skill-lifecycle-test/SKILL.md         # 内部 — 产出 test report（入口条件：plan doc 存在）
...
1. 列出 5 个阶段（design / plan / test / review / feedback）和当前各阶段产物状态
2. 问用户要进哪个
3. 检查入口条件：
...
| test | `skill-lifecycle-router/test/<skill-name>-test-report.md` | plan doc 存在 | "没有实现计划就测试是在盲目验证——先用 plan 阶段明确要验证什么" |
...
TDD 发现文档缺口   → feedback 记录（标注回流: design）→ 回到 design 补充文档
...
- **TDD 循环**：两层嵌套审查。第一层：**测试审代码**——Agent 先写测试，再写代码，测试是代码的自动化审查者。第二层：**反查审测试**——如果测试全通过但 codebase 有肉眼可见问题，Harness 判定测试无效，强制 Agent 重新考虑测试边界。对应 Harness 原文"AI 写的测试通过了有 bug 的代码 → 判定测试无效"。
...
`test` 的反查规则直接来自 Harness 实践：如果 AI 写的测试通过了有 bug 的代码，Harness 判定测试无效，强制 Agent 重新考虑测试边界。
...
Router 只检查声明**存在**，不校验声明内容的正确性——校验质量是 review 阶段的事。缺少自检声明视为阶段未完成。
