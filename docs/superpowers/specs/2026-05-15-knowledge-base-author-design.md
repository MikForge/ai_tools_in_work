...
- 建议标题。
- 建议摘要。
- 可选分类建议，且必须来自 `.knowledge-base.yml` 已配置分类；该建议不能视为最终发布分类。
...
- 分类建议只能使用 contract 中的配置分类；无法判断时输出 “no category suggestion”。
- 若材料不足以成文，提出缺失信息，不编造。
...
```text
.agents/skills/knowledge-base-router/knowledge-base-author/
├── SKILL.md
├── zh-CN.md
└── agents/
    └── openai.yaml
