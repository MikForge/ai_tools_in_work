...
1. 加载 workflow 索引。
2. 通过 `BaseContext/INDEX.md` 选择 repo-first 探测需要的架构参考。
3. 先探测仓库已有资源，再向用户提问。
4. 展示探测结果，等待用户确认或修正。
5. 只追问仓库无法确定的模块事实。
6. 通过 `constraint/INDEX.md` 标记本模块适用的约束。
7. 将模板渲染到 `docs/mvc-workflows/<ModuleName>/`。
8. 初始化 `_context.md`、`_tree.md`、`_status.md`、`_constraint.md` 和 `_dev_log.md`。
9. 报告生成的工作区和下一步动作。
