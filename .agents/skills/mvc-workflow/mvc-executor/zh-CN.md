...
- 不得压制或忽略 03_execute 期间的 IDE 诊断错误。每个错误都必须先记录到 `_dev_log.md` 和 `_status.md`，再修复。
...
6. 按 `_tree.md` 顺序执行活动层；如模块需要命令工作，也执行可选 `Command` 层。
7. 每个活动层执行 `01_spec -> 02_plan -> 03_execute -> 04_test`。
   - **03_execute 错误记录**：每次源码编辑后检查 hook response 中的 IDE 诊断。若存在错误：
     1. 在 `_dev_log.md` 记录 `execute_fail`，包含完整错误码和信息。
     2. 将当前层 `_status.md` 置为 `failed`，递增 `retries`，写入 `last_error`。
     3. 追踪根因；如果来自上游层，在 `_dev_log.md` 记录 `root_cause` 并更新对应层状态。
     4. 修复后记录 `fix` 和 `execute_retry`。
     5. 仅在相关诊断清空或明确 blocked 后继续。
8. 每次转换后更新 `_status.md`；层变为 `done` 或 `skipped` 时清空 `last_error`。
9. 每次状态变化后追加 `_dev_log.md`。
10. 所有层为 `done` 或 `skipped` 后执行集成验证，并将结果持久化到 `_status.md`。
